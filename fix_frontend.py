...
ssh.exec_command('pkill -f "next" 2>/dev/null || true')

# Write a clean, full page.tsx
...
export default function Home() {
  ...
  const fetchNews = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const res = await fetch(`http://103.175.219.57:8085/api/news?page=${p}`);
...
sftp = ssh.open_sftp()
import io
sftp.putfo(io.BytesIO(page_content.encode('utf-8')), '/root/news-hybrid/frontend/src/app/page.tsx')

# Also fix next.config.ts to allow external images
...
"""
sftp.putfo(io.BytesIO(next_config.encode('utf-8')), '/root/news-hybrid/frontend/next.config.ts')
sftp.close()

print("Files written. Restarting Next.js on port 3000...")
ssh.exec_command('pkill -f "next" 2>/dev/null; sleep 2')
ssh.exec_command('cd /root/news-hybrid/frontend && nohup npm run dev -- -p 3000 > next.log 2>&1 &')
print("Next.js started! Waiting 15 seconds for it to compile...")
import time
time.sleep(15)

stdin, stdout, stderr = ssh.exec_command('tail -20 /root/news-hybrid/frontend/next.log')

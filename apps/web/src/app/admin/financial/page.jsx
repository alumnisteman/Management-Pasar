...
// ── Stat Card ──────────────────────────────────────────────────────────────────
function KpiCard({ icon: Icon, label, value, sub, trend, iconClass, accent, delay = 0 }) {
  return (
    <motion.div
    ...
    >
      ...
      {trend !== undefined && (
        <div className={twMerge("flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded-full shrink-0 mt-1", trend > 0 ? "bg-green-500/10 text-green-400" : trend < 0 ? "bg-red-500/10 text-red-400" : "bg-gray-500/10 text-gray-400")}>
...
// ── Zone Card ──────────────────────────────────────────────────────────────────
const ZONE_META = {
  gold: { label: "Gold", color: "#eab308", bg: "bg-yellow-500/10", border: "border-yellow-500/20", text: "text-yellow-400", ring: "from-yellow-500/20 to-yellow-500/5" },
  silver: { label: "Silver", color: "#9ca3af", bg: "bg-gray-400/10", border: "border-gray-400/20", text: "text-gray-400", ring: "from-gray-400/20 to-gray-400/5" },
  bronze: { label: "Bronze", color: "#b45309", bg: "bg-orange-700/10", border: "border-orange-700/20", text: "text-orange-600", ring: "from-orange-700/20 to-orange-700/5" },
};
...
function ZoneCard({ data, delay }) {
  ...
  return (
    ...
    >
      <div className={twMerge("absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl -mt-8 -mr-8 bg-gradient-radial", meta.ring)} />
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          <p className="text-[10px] text-gray-500">Terkumpul</p>
          <p className="text-base font-bold text-white">{fmtRpShort(data.collected)}</p>
        </div>
        <div>
          <p className="text-[10px] text-gray-500">Piutang</p>
          <p className="text-base font-bold text-red-400">{fmtRpShort(data.unpaid)}</p>
        </div>
        <div>
          <p className="text-[10px] text-gray-500">Lapak Terisi</p>
          <p className="text-base font-bold text-white">{data.occupiedStalls}/{data.totalStalls}</p>
        </div>
        <div>
          <p className="text-[10px] text-gray-500">Tarif/Bulan</p>
          <p className="text-base font-bold text-white">{fmtRpShort(data.monthlyFee)}</p>
        </div>
      </div>
      <div>
        <div className="flex justify-between text-[10px] text-gray-500 mb-1">
          <span>Hunian</span><span>{occupancy}%</span>
        </div>
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${occupancy}%`, background: meta.color }} />
        </div>
      </div>
    </motion.div>
  );
...
export default function FinancialPage() {
  ...
  const hasPrediction = cashflowChart?.some(m => m.isPrediction);
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className={twMerge(
          "hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold",
          ...
          {kpis.trendDirection === 'up' ? <TrendingUp size={16} /> : kpis.trendDirection === 'down' ? <TrendingDown size={16} /> : <Minus size={16} />}
      ...
      <SectionCard
        title="Cashflow & Prediksi Pendapatan"
        icon={BarChart3}
            ...
            <div className="space-y-3">
              {topDebtors.map((trader, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-white/3 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                  <div className={twMerge("w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0", i === 0 ? "bg-red-500/20 text-red-400" : i === 1 ? "bg-orange-500/20 text-orange-400" : "bg-gray-500/20 text-gray-400")}>
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">

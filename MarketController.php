...
class MarketController extends Controller
{
    ...
    public function checkPerformanceAlerts()
    {
        ...
        $actual = DB::table('transactions')
            ...
            ->where(DB::raw('HOUR(transaction_time)'), '<=', $currentHour)
            ->sum('amount');
        ...
        if ($actual < ($forecast * 0.7)) {
            Notification::create([
                ...
                'message' => 'Pendapatan hari ini (Rp ' . number_format($actual) . ') berada 30% di bawah target (Rp ' . number_format($forecast) . '). Segera lakukan audit lapangan.',
                'type' => 'alert',
                'is_read' => false
            ]);
    ...
    public function monthlyReport(Request $request)
    {
        ...
        $report = DB::table('transactions')
            ->select(DB::raw('DATE(transaction_time) as date'), DB::raw('SUM(amount) as revenue'), DB::raw('COUNT(*) as count'))
    ...
    public function revenueAnalytics()
    {
        ...
        $data = DB::table('transactions')
            ->select(DB::raw('HOUR(transaction_time) as hour'), DB::raw('SUM(amount) as revenue'))
            ->whereDate('transaction_time', today())
            ->groupBy('hour')
            ->orderBy('hour')
            ->get();
    ...
    public function verifyPayment($traderId)
    {
        ...
        if ($lastPayment) {
            ...
            $isPaid = $paidDate->diffInDays(now()) < 30;
        }
    ...
    public function commandCenterStats()
    {
        $totalVendors = Trader::count();
        $activeVendors = Trader::where('status', 'active')->count();
        $complianceRate = $totalVendors > 0 ? round(($activeVendors / $totalVendors) * 100) : 0;
        ...
        $revenueToday = DB::table('transactions')->whereDate('transaction_time', today())->sum('amount');
        $revenueMonth = DB::table('transactions')->whereMonth('transaction_time', now()->month)->sum('amount');
        ...
        return response()->json([
            ...
            'active_complaints' => \App\Models\WhistleblowerReport::count(),
            'total_traders' => $totalVendors
        ]);
    }
}

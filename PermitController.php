...
class PermitController extends Controller
{
    ...
    {
        $permit = Permit::with(['trader', 'slot'])->where('permit_number', $permitNumber)->first();
        
        if (!$permit) {
            return response()->json(['status' => false, 'message' => 'Izin Tidak Ditemukan'], 404);
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'market' => 'Pasar Pusat Kendari', // Default or from relationship
                'stall' => $permit->slot->code,
                'status' => $permit->status,
                'issued' => $permit->issued_at,
                'expires' => $permit->expires_at,
                'is_valid' => ($permit->status === 'active' && now()->lte($permit->expires_at)),
            ]
        ]);
    }
    ...
    public function renew(Request $request, $id)
    {
        ...
        $permit->expires_at = $request->new_expires_at;
        $permit->status = 'active'; // Reactivate if expired
        $permit->save();

        AuditLogger::log('RENEW_PERMIT', [
            'permit_id' => $permit->id,
            'new_expiry' => $permit->expires_at
        ]);
        ...
        if ($trader) {
            \App\Jobs\SendWhatsAppNotification::dispatch(
                ...
                "Masa berlaku SIPTU Anda telah diperpanjang hingga {$permit->expires_at}. Silakan unduh kartu terbaru.",
                ['permit' => $permit->permit_number]
            );
        }

        return response()->json(['status' => 'success', 'expires_at' => $permit->expires_at]);
    }
}
